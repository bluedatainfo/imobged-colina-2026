...
const ManagerApproval = () => {
  ...
  const handleApprove = (id: string) => {
    ...
    mainStore.addAuditLog({ propertyId: id, action: 'Aprovação Gerencial', user: user?.name || 'Sistema', details: 'Documentação validada. Handoff para vistoria.' })
    toast({
  ...
  const handleRejectConfirm = () => {
    ...
    if (rejectId) {
      ...
      mainStore.addAuditLog({ propertyId: rejectId, action: 'Documentação Rejeitada', user: user?.name || 'Sistema', details: `Motivo: ${rejectReason}` })
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Aprove a documentação de novos inquilinos para liberar a Vistoria de Entrada. Fique atento aos prazos SLA.</p>
      </div>
      ...
      <div className="grid gap-4">
        {approvals.map((item) => {
          ...
          return (
            <Card key={item.id} className={`flex flex-col xl:flex-row gap-4 p-4 items-start ${breached ? 'border-destructive bg-destructive/5' : ''}`}>
              <div className="flex-1 space-y-4 w-full">
                <div className="flex items-start gap-3">
                  <div className={`p-2.5 rounded-full shrink-0 ${breached ? 'bg-destructive/20' : 'bg-primary/10'}`}>
                    <UserCheck className={`h-5 w-5 ${breached ? 'text-destructive' : 'text-primary'}`} />
                  </div>
                  <div>
                    ...
                    <div className="flex gap-2 pt-2 flex-wrap">
                      <Badge variant="outline" className="border-amber-500 text-amber-600 bg-amber-50">Análise Gerencial</Badge>
                      {breached && (
                        <Badge variant="destructive" className="animate-pulse">
                          <AlertCircle className="w-3 h-3 mr-1" /> SLA Violado (> {store.settings.slaHours}h)
                        </Badge>
                      )}
              ...
              <div className="flex xl:flex-col gap-2 w-full xl:w-48 xl:border-l xl:pl-4 xl:border-border">
                <Button className="w-full bg-emerald-600 hover:bg-emerald-700 text-white" onClick={() => handleApprove(item.id)}>
      ...
      <Dialog open={!!rejectId} onOpenChange={(val) => !val && setRejectId(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Informe o motivo da rejeição. Este feedback será enviado ao responsável e à equipe administrativa.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            <Textarea placeholder="Ex: Faltou enviar o verso do RG ou o comprovante está ilegível..." value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} className="min-h-[100px]" />
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setRejectId(null)}>Cancelar</Button>
            <Button variant="destructive" onClick={handleRejectConfirm}>Confirmar Rejeição</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
