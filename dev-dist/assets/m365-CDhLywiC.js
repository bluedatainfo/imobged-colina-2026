...
var m365Service = {
	...
	findDocumentInSharePoint: async (fileName) => {
		...
		const envBaseUrl = "https://ismailabdo.sharepoint.com/sites/locacoes/imoveis";
	...
	getFilePreviewUrl: async (filePath, documentType) => {
		...
		const fullExpectedUrl = `${envBaseUrl.replace(/\/$/, "")}/${config.library_name}/${safePath}`;
	...
	fetchListItems: async (sitePath, listName) => {
		...
		if (!token || !clientId || !tenantId) {
			if (listName.includes("PF") || listName.includes("Locat") || listName.includes("Locatrios")) return [{
				id: "mock-1",
				fields: {
					Title: "João Silva",
					CPF: "111.222.333-44",
			...
			if (listName.includes("PJ")) return [{
				id: "mock-2",
				fields: {
			...
			if (listName.includes("Fiador")) return [{
				id: "mock-3",
				fields: {
...
//# sourceMappingURL=m365-CDhLywiC.js.map