...
var m365Service = {
	...
	listTemplates: async () => {
		...
		try {
			...
			const drivesRes = await fetchWithAuth(`https://graph.microsoft.com/v1.0/sites/${siteId}/drives`);
			if (!drivesRes.ok) return [];
			const drive = (await drivesRes.json()).value.find((d) => d.name && d.name.toLowerCase().includes("documento"));
			if (!drive) return [];
			const res = await fetchWithAuth(`https://graph.microsoft.com/v1.0/sites/${siteId}/drives/${drive.id}/root:/${folderPath}:/children`);
			if (res.ok) return (await res.json()).value?.map((item) => ({
				...item,
				siteId,
				driveId: drive.id
			})) || [];
	...
	downloadItemContent: async (siteId, driveId, itemId) => {
		if (!getGraphToken()) throw new Error("Sessão M365 ausente.");
		const res = await fetchWithAuth(`https://graph.microsoft.com/v1.0/sites/${siteId}/drives/${driveId}/items/${itemId}/content`);
	...
	uploadStructuredDocument: async (file, fileName, documentType, propertyId, propertyTitle, userName, entityCode, entityName, leaseNumber, folderNumber) => {
		...
		let targetFolder = "";
		if (isEntityDoc && entityCode) targetFolder = [basePath, entityCode].filter(Boolean).join("/");
		else if (isInspection && leaseNumber) targetFolder = [
			basePath,
			propertyId,
			"Locacao",
			leaseNumber,
		...
		else if (isLease && leaseNumber) targetFolder = [
			basePath,
			propertyId,
			"Locacao",
			leaseNumber
		].filter(Boolean).join("/");
		else if (folderNumber) targetFolder = [basePath, folderNumber].filter(Boolean).join("/");
		else targetFolder = [basePath, propertyId].filter(Boolean).join("/");
...
//# sourceMappingURL=m365-CBkwAIgl.js.map