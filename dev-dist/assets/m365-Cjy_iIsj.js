	...
	uploadStructuredDocument: async (file, fileName, documentType, propertyId, propertyTitle, userName) => {
		...
		try {
			if (!token || !clientId || !tenantId) toast({
				...
				const url = `https://graph.microsoft.com/v1.0/sites/${siteId}/drives/${drive.id}/root:/${fullPath}:/content`;
				if (!(await fetch(url, {
					method: "PUT",
					headers: {
						Authorization: `Bearer ${token}`,
					...
					body: file
				})).ok) throw new Error("Erro de permissão no SharePoint ou caminho de upload inválido.");
			}
...
//# sourceMappingURL=m365-Cjy_iIsj.js.map