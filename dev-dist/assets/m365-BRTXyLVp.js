...
import { r as mainStore } from "./main-_gP0LOX9.js";
...
var m365Service = {
	...
	findDocumentInSharePoint: async (fileName) => {
		...
		try {
			...
			if (sitePath.startsWith("http")) try {
				sitePath = new URL(sitePath).pathname;
			} catch (e) {}
			if (!sitePath.startsWith("/")) if (!sitePath.startsWith("sites/") && !sitePath.startsWith("teams/")) sitePath = `/sites/${sitePath}`;
			...
			if (siteRes.status === 404 && sitePath.startsWith("/sites/")) {
				siteUrl = `https://graph.microsoft.com/v1.0/sites/${sharepointDomain}:${sitePath.replace("/sites/", "/teams/")}`;
	...
	getFilePreviewUrl: async (filePath, documentType) => {
		...
		let siteRes = await fetchWithAuth(`https://graph.microsoft.com/v1.0/sites/${sharepointDomain}:${sitePath}`);
		if (siteRes.status === 404 && sitePath.startsWith("/sites/")) {
			const fallbackRes = await fetchWithAuth(`https://graph.microsoft.com/v1.0/sites/${sharepointDomain}:${sitePath.replace("/sites/", "/teams/")}`);
			if (fallbackRes.ok) siteRes = fallbackRes;
		}
		if (!siteRes.ok) throw new Error(`Site M365 "${config.site_name}" não encontrado. Verifique o mapeamento GED.`);
		const siteId = (await siteRes.json()).id;
		const drivesRes = await fetchWithAuth(`https://graph.microsoft.com/v1.0/sites/${siteId}/drives`);
		...
		if (!itemRes.ok) throw new Error(`Arquivo não encontrado no SharePoint no caminho: ${safePath}`);
	...
	saveToLibrary: async (library, fileName, fileContent = "Conteúdo gerado via sistema", sitePath = "locacao") => {
		...
		try {
			...
			let siteRes = await fetchWithAuth(`https://graph.microsoft.com/v1.0/sites/${hostname}:${spPath}`);
			if (siteRes.status === 404 && spPath.startsWith("/sites/")) {
				const fallbackPath = spPath.replace("/sites/", "/teams/");
				const fallbackRes = await fetchWithAuth(`https://graph.microsoft.com/v1.0/sites/${hostname}:${fallbackPath}`);
				if (fallbackRes.ok) {
					siteRes = fallbackRes;
					spPath = fallbackPath;
				}
	...
	uploadStructuredDocument: async (file, fileName, documentType, propertyId, propertyTitle, userName, entityCode, entityName) => {
		...
		try {
			if (!token || !clientId || !tenantId) {
				...
				let siteRes = await fetchWithAuth(`https://graph.microsoft.com/v1.0/sites/${hostname}:${sitePath}`);
				if (siteRes.status === 404 && sitePath.startsWith("/sites/")) {
				...
				if (!siteRes.ok) {
					const errorMsg = await siteRes.json().catch(() => ({ error: { message: siteRes.statusText } }));
					throw new Error(`Site M365 "${config.site_name}" não encontrado. (Erro: ${errorMsg?.error?.message || siteRes.status}). Verifique o mapeamento GED.`);
				}
				const siteId = (await siteRes.json()).id;
				const drivesRes = await fetchWithAuth(`https://graph.microsoft.com/v1.0/sites/${siteId}/drives`);
				if (!drivesRes.ok) throw new Error(`Não foi possível listar as bibliotecas do site "${config.site_name}".`);
...
//# sourceMappingURL=m365-BRTXyLVp.js.map