...
var initMainStore = async () => {
	...
	if (properties && properties.length > 0) state.properties = properties.map((p) => ({
		...
		for (const p of initialProps) await supabase.from("properties").insert({
			id: p.id,
			title: p.title,
			address: p.address,
			type: p.type,
...
var mainStore = {
	...
	reloadCoreConfig: async () => {
		try {
			const { data: coreData } = await supabase.from("app_settings").select("*").limit(1).maybeSingle();
...
//# sourceMappingURL=main-BrkNkJx4.js.map