...
var initDocumentsStore = async () => {
	...
	if (data && data.length > 0) state.documents = data.map((d) => ({
		...
		reviewNotes: d.review_notes || void 0
	}));
	emit();
};
...
var documentsStore = {
	...
	addDocument: async (doc) => {
		...
		if (data) {
			const newDoc = {
				...
				reviewNotes: data.review_notes || void 0
			};
			state = {
				...state,
	...
	updateReviewNotes: async (id, notes) => {
		const { error } = await supabase.from("property_documents").update({ review_notes: notes }).eq("id", id);
		if (!error) {
			state = {
				...state,
...
//# sourceMappingURL=documents-BY1Fyi1z.js.map