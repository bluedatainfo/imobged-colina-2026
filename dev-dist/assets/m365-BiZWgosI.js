...
var m365Service = {
	...
	getDriveItemChildren: async (siteId, driveId, itemId) => {
		...
		try {
			const res = await fetchWithAuth(`https://graph.microsoft.com/v1.0/sites/${siteId}/drives/${driveId}/items/${itemId}/children`);
			if (res.ok) {
				const data = await res.json();
				return data.value ? data.value.map((v) => ({
					...v,
					siteId,
					driveId
				})) : [];
...
//# sourceMappingURL=m365-BiZWgosI.js.map